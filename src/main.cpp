...
#define FAZA_START 0
#define FAZA_WZNOSZENIE 1
#define FAZA_OPADANIE 2
#define FAZA_WYLADOWAL 3

rfm22 radio1(10);
char datastring[200];
unsigned int pkt_num;
unsigned int numerRamki = 0;
float battvolt = 0;
...
void rtty_send()
{
  ...
  strcat(datastring, temp1);
    ...
    strcat(datastring, temp1);
    //faza lotu, temperatura2
    strcat(datastring, ",");
    snprintf(temp1, 2, "%01d", (int)fazaLotu);
    strcat(datastring, temp1);
    strcat(datastring, ",");
    dtostrf(tempout, 3, 1, temp1); // temp_in
    strcat(datastring, temp1);
...
void sensors()
{
  ...
  Serial.println(temp1);
  Serial.flush();
  tempin = wewnetrzna_temperaturaDS();
  tempout = zewnetrzna_temperaturaDS();
}
...
void ustalFazeLotu(float wysokosc)
{
    if(numerRamki % 3 == 0)
    {
      wysokosc0 = wysokosc;
    }
    else if(numerRamki % 3 == 1)
    {
      wysokosc1 = wysokosc;
    }
    else if(numerRamki % 3 == 2)
    {
      if(int(wysokosc0) == int(wysokosc1) && int(wysokosc0) == int(wysokosc) && fazaLotu != FAZA_START)
      {
        fazaLotu = FAZA_WYLADOWAL;
      }
      else if(int(wysokosc0) < int(wysokosc1) && int(wysokosc1) < int(wysokosc))
      {
        fazaLotu = FAZA_WZNOSZENIE;
      }
      else if(int(wysokosc0) > int(wysokosc1) && int(wysokosc1) > int(wysokosc))
      {
        fazaLotu = FAZA_OPADANIE;
      }
    }
}
...
void loop()
{
  ...
  if (czasOdOstatniegoTX >= RTTY_PERIOD)
  {
    blink(active ? 1 : 2);
    sensors();
    ustalFazeLotu(gps_altitude);
    rtty_send();
    zapisznaSD(datastring+14);
    

    if (fazaLotu == FAZA_WYLADOWAL) {
